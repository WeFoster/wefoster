...
function mobile_defaults() {
	...
	if ( ! $tmp ) {
		update_option( 'mobile_body_class', 1 );
	}
}
...
function wf_mobile_body_class( $classes ) {
	...
	if ( $mobile_detect->is( 'Gecko' ) ) {
		$classes[] = "gecko";
	}
	if ( $is_opera ) {
		$classes[] = "opera";
	}
	if ( $mobile_detect->is( 'Opera' ) ) {
		$classes[] = "opera";
	}
	if ( $is_NS4 ) {
		$classes[] = "ns4";
	...
	if ( $mobile_detect->is( 'Safari' ) ) {
		$classes[] = "safari";
	}
	if ( $is_chrome ) {
		$classes[] = "chrome";
	}
	if ( $mobile_detect->is( 'Chrome' ) ) {
		$classes[] = "chrome";
	}
	if ( $is_IE ) {
		$classes[] = "ie";
	}
	if ( $mobile_detect->is( 'IE' ) ) {
		$classes[] = "ie";
	}
