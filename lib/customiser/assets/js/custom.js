...
(function($) {
  ...
  wp.customize('wf_plus_footer_style', function(value) {
    ...
    wp.customize('wff_preset_demo', function(value) {
  ...
  wp.customize('wf_plus_bp_cover_photo_effect', function(value) {
    value.bind(function(to) {
      ...
      iframe.find('.bp-cover-image-wrap').attr('class', 'bp-cover-image-wrap');
      iframe.find('.bp-cover-image-wrap').addClass(to);
  ...
  //Add Image Effect Class to Body Background
  wp.customize('wf_body_background_image_color', function(value) {
    value.bind(function(to) {
      var iframe = $('.wp-full-overlay-main iframe').contents();

      iframe.find('.wefoster-plus-background-overlay').attr('class', 'wefoster-plus-background-overlay');
      iframe.find('.wefoster-plus-background-overlay').addClass(to);
    });
  });
  ...
  if ('undefined' !== typeof prefixL10n) {
    ...
    stylekit = $('<a class="stylekit-info-link feature-description tooltip hint--left"></a>')
      .attr('href', prefixL10n.prefixURL)
      .attr('target', '_blank')
      .attr('data-hint', 'Choose from a constantly growing set of carefully crafted designs developed from the ground up for community interaction. Switch designs with the click of a button. As a WeFoster Plus subscriber you get unlimited access to all our premium designs and the revolutionary BuddyBuilder.')
    ...
    setTimeout(function() {
      $('#accordion-section-wf_plus_typography_navigation_section').append(typography);
    }, 200);
